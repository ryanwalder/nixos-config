{
  ...
  # builtins.fromJSON parses JSON \uXXXX escapes, giving us actual Unicode chars
  # Wrapped in %{T2}...%{T-} to use font-1 (larger icon font)
  icon = glyph: "%{T2}${builtins.fromJSON glyph}%{T-}";
  i = {
    cpu    = icon ''"\uF2DB"''; # nf-fa-microchip
    memory = icon ''"\uF538"''; # nf-fa-memory
    disk   = icon ''"\uF0A0"''; # nf-fa-hdd
    net    = icon ''"\uF796"''; # nf-fa-ethernet
    vol0   = icon ''"\uF026"''; # nf-fa-volume-off
    vol1   = icon ''"\uF027"''; # nf-fa-volume-down
    vol2   = icon ''"\uF028"''; # nf-fa-volume-up
    mute   = icon ''"\uF6A9"''; # nf-fa-volume-mute
  };
  ...
  launchPolybar = pkgs.writeShellScriptBin "polybar-launch" ''
    # Prevent concurrent runs (e.g. i3 always=true + manual call racing)
...
in {
  ...
  config = {
    ...
    # Disable i3's default bar — polybar replaces it
    xsession.windowManager.i3.config.bars = lib.mkForce [];

    # Launch polybar on i3 startup and every reload
