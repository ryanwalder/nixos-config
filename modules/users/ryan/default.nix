{ inputs, ... }: {
  flake.modules.nixos.user_ryan = { config, lib, ... }: {
    users.users.ryan = {
      uid = 1000;
      isNormalUser = true;
      extraGroups = [ "dialout" "wheel" ] ++ lib.optional config.virtualisation.docker.enable "docker";
      openssh.authorizedKeys.keys = [
        "ssh-rsa  YUBIKEY-PERSONAL-A"
      ];
    };

    home-manager.users.ryan.imports = with inputs.self.modules.homeManager; [
      user_ryan
    ];
  };

  flake.modules.homeManager.user_ryan = {
    home.username = "ryan";
    home.stateVersion = "25.11";
  };
}
